...
class AssemblyConverter:
		...
		for i in range(len(self.code)):
			if x+":" == self.code[i]:
				if i>line_num:
					...
					return (i+1- line_num)*4

				 #how many instructions to jump ahead/behind
		#print("Address not found")
		return int(x) #if not found
	...
	def __binary(self, x, size):
		...
		try:
			...
			b_num = x.to_bytes(byte_num, byteorder = 'big', signed = False)	
...
# Removing char at pos 3
	...
	def SB_type(
		...
		mod_imm = (int(imm) - ((int(imm) >> 13) << 13)) >> 12 # imm[12]
		
		
		mod_imm_2= (int(imm) - ((int(imm) >> 11) << 11)) >> 5 # imm[12|10:5]
		
		
		mod_imm_3 = (int(imm) - ((int(imm) >> 5) << 5))>>1 # imm[4:1]
		mod_imm_4 = (int(imm) - ((int(imm) >> 12) << 12)) >> 11 # imm[4:1|11]
		...
		return "".join([
			...
			#]),
			self.__binary(mod_imm,1),
			self.__binary(mod_imm_2,6),
			self.__reg_to_bin(rs2),
			self.__reg_to_bin(rs1),
			self.instr_data[instr][f3],
			self.__binary(mod_imm_3,4),
			self.__binary(mod_imm_4,1),
			#"".join([
	...
	def UJ_type(
		...
		mod_imm_1 = (int(imm) - ((int(imm) >> 21) << 21)) >> 20 # imm[20]
		mod_imm_2= (int(imm) - ((int(imm) >> 11) << 11)) >> 1 # imm[20|10:1]
		print(mod_imm_2)
		mod_imm_3= (int(imm) - ((int(imm) >> 12) << 12)) >> 11 # imm[20|10:1|11]
		mod_imm_4= (int(imm) - ((int(imm) >> 20) << 20)) >> 12 # imm[20|10:1|11|19:12]
		...
		return  "".join([
			...
			#]),		
			self.__binary(mod_imm_1,1),
			self.__binary(mod_imm_2,10),
			self.__binary(mod_imm_3,1),
			self.__binary(mod_imm_4,8),
			self.__reg_to_bin(rd),
			self.instr_data[instr][opcode]
		])
	...
	def __interpret(self,line,i):
		...
		if clean[0] in self.R_instr:
			...
			if len(clean) == 3:
				res.append(self.UJ_type(clean[0],clean[2], self.__reg_map(clean[1])))
			else:
	...
	def __post(self):
		...
		print("\n\nConverting Assembly into Hex..." )
		print("No of instructions: {}".format(len(self.instructions)))
